...
public class DataUtility {
   ...
   */
  static Record createRecord(SinkRecord sinkRecord) {
    String recordSeparator = "\n";
    try {
      String valueAsString = OBJECT_MAPPER.writer().writeValueAsString(sinkRecord.value());
      valueAsString = valueAsString + recordSeparator;
      ByteBuffer bytebuffer = ByteBuffer.wrap(valueAsString.getBytes(StandardCharsets.UTF_8));
      return new Record().withData(bytebuffer);
